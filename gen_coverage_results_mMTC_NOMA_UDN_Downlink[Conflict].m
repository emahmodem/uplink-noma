...
params.PD = params.Pdl /  params.N_RB ;  % DL transmit power per radio block
params.BW = 20e6;
...
params.rho_m = 0.3;
                 % HTC users density  (users/m^2)


params.H = exprnd(1,6e2,2e5);
...
switch(sim)
    ...
    case 'DL_Coverage_SmallCellsDensity_C2A'
        ...
        params.simulation_area_side = [-2500 2500];    % simulation area side to side
...
%         params.SEPL.beta = 1/2;
        ...
        params.LA_B = [1000 10000 100000]*1e-6 ;       % Small Cell Denisty (cells/m^2)
        LA_H = [100]*1e-6 ;
        for i = 1:numel(LA_H)
            params.LA_H = LA_H(i);
