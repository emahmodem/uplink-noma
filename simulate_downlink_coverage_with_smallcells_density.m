function [Pa, Pcov_h, Pcov_m] = simulate_downlink_coverage_with_smallcells_density(params)
...
PA = zeros(1,params.space_realizations );
for p = 1:points
    %     if(params.LA_B(p) <= 1000)
    %         params.simulation_area_side = [-3000 3000];
    %     elseif( params.LA_B(p) > 1000 && params.LA_B(p) <= 5000  )
    %         params.simulation_area_side = [-1500 1500];
    %     elseif( params.LA_B(p) > 5000 && params.LA_B(p) <= 10000  )
    %         params.simulation_area_side = [-1000 1000];
    %     end
    %    simulation_area =  (params.simulation_area_side(2) - params.simulation_area_side(1))^2;
    fprintf('\n')
    %disp(['coverage Threshold: ' , num2str(params.Threshold(p))]);
    disp(['Smallcell Density: ' , num2str(params.LA_B(p)*1e6)]);
    ...
    for m = 1:params.space_realizations;
        ...
        for t = 1:params.time_slots
            ...
            %             R_h =  params.PD *   exp(- params.SEPL.alpha.*distances.HTC_to_BS.^params.SEPL.beta);
            %             R_m =  params.PD *   exp(- params.SEPL.alpha.*distances.MTC_to_BS.^params.SEPL.beta);
