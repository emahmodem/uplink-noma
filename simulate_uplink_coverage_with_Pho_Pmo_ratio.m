function [Pcov_h, Pcov_m] = simulate_uplink_coverage_with_Pho_Pmo_ratio(params)
simulation_area =  (params.simulation_area_side(2) - params.simulation_area_side(1))^2;
points = numel(params.MTC.Pmin);
...
for p = 1:points
fprintf('\n')
disp(['MTC Minimum Power: ' , num2str(params.MTC.Pmin(p))]);
...
for m = 1:params.space_realizations;
    ...
    % Power Control
    server_MTC_P = params.MTC.Pmin(p) * exp(params.SEPL.alpha .* distances.MTC_to_Server .^ params.SEPL.beta);
...
%         end
        ...
        for user_m = 1:N_users_MTC
            ...
            S_m(user_m) = params.MTC.Pmin(p) * H_m(server_m,user_m);
