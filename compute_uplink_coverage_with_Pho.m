function [Pcov_h, Pcov_m] = compute_uplink_coverage_with_Pho(params)
...
for k = 0:n
    ...
    for p = 1:numel(Pho)
        if(th < Pho(p)/Pmo)
            ...
            J_vh_m_r1_t1(k+1,p) = integral(@(y)F(y,n - k,2*delta_h(p).*(Pmo/Pho(p))),0,2*delta_h(p)*(Pmo/Pho(p)));
            J_vh_m_r1_t2(k+1,p) = integral(@(y)F(y,n - k,th*(Pmo/Pho(p))),0,th*(Pmo/Pho(p)));
            
            T_h_h_r1_t1(k+1,p) = NK * J_vh_h_r1_t1(k+1,p) * Eph(k+1,p) ;
            T_h_h_r1_t2(k+1,p) = NK * J_vh_h_r1_t2(k+1,p) * Eph(k+1,p) ;
            T_h_m_r1_t1(k+1,p) = NK * J_vh_m_r1_t1(k+1,p) * Epm(k+1) ;
            T_h_m_r1_t2(k+1,p) = NK * J_vh_m_r1_t2(k+1,p) * Epm(k+1) ;
        else
            J_vh_h_r2(k+1,p) = integral(@(y)F(y,n - k,th),0,th);
            J_vh_m_r2(k+1,p) = integral(@(y)F(y,n - k,th*(Pmo/Pho(p))),0,th*(Pmo/Pho(p)));
            
            T_h_h_r2(k+1,p) = NK * J_vh_h_r2(k+1,p) * Eph(k+1,p) ;
            T_h_m_r2(k+1,p) = NK * J_vh_m_r2(k+1,p) * Epm(k+1) ;
        end
        
        if(th_qos * Pmo/Pho(p) - 1.0 >=0 )
            J_vm_m_T1(k+1,p) = integral(@(y)F(y,n - k,epsi(p)*(Pmo/Pho(p))),0,epsi(p)*(Pmo/Pho(p)));
            J_vm_h_T1(k+1,p) = integral(@(y)F(y,n - k,epsi(p)),0,epsi(p));
            
            T_m_m_T1(k+1,p) = NK * J_vm_m_T1(k+1,p) * Epm(k+1) ;
            T_m_h_T1(k+1,p) = NK * J_vm_h_T1(k+1,p) * Eph(k+1,p) ;
        else
            if(tm < Pmo/Pho(p) * delta_h_qos(p))
                J_vm_m_T1(k+1,p) = 0;
                J_vm_h_T1(k+1,p) = 0;
                J_vm_m_r1_t1(k+1,p) = integral(@(y)F(y,n - k,2*delta_h_qos(p)*(Pmo/Pho(p))),0,2*delta_h_qos(p)*(Pmo/Pho(p)));
                J_vm_m_r1_t2(k+1,p) = integral(@(y)F(y,n - k,epsi(p)*(Pmo/Pho(p))),0,epsi(p)*(Pmo/Pho(p)));
                ...
                T_m_m_r1_t1(k+1,p) = NK * J_vm_m_r1_t1(k+1,p) * Epm(k+1) ;
                T_m_m_r1_t2(k+1,p) = NK * J_vm_m_r1_t2(k+1,p) * Epm(k+1) ;
                T_m_h_r1_t1(k+1,p) = NK * J_vm_h_r1_t1(k+1,p) * Eph(k+1,p) ;
                T_m_h_r1_t2(k+1,p) = NK * J_vm_h_r1_t2(k+1,p) * Eph(k+1,p) ;
            else
                J_vm_m_r2(k+1,p) = integral(@(y)F(y,n - k,2*tm),0,2*tm);
                J_vm_h_r2(k+1,p) = integral(@(y)F(y,n - k,2*tm*(Pho(p)/Pmo)),0,2*tm*(Pho(p)/Pmo));
                
                T_m_m_r2(k+1,p) = NK * J_vm_m_r2(k+1,p) * Epm(k+1) ;
                T_m_h_r2(k+1,p) = NK * J_vm_h_r2(k+1,p) * Eph(k+1,p) ;
            end
        end
    end
    %     T_h_h_r1_t1(k+1,:) = NK * J_vh_h_r1_t1(k+1,:) * Eph(k+1,p) ;
    %     T_h_h_r1_t2(k+1,:) = NK * J_vh_h_r1_t2(k+1,:) * Eph(k+1,p) ;
    %     T_h_h_r2(k+1,:) = NK * J_vh_h_r2(k+1,:) * Eph(k+1,p) ;
    ...
    %     T_m_h_T1(k+1,:) = NK * J_vm_h_T1(k+1,:) * Eph(k+1,p) ;
    %
    ...
    %     T_m_h_r1_t1(k+1,:) = NK * J_vm_h_r1_t1(k+1,:) * Eph(k+1,p) ;
    %     T_m_h_r1_t2(k+1,:) = NK * J_vm_h_r1_t2(k+1,:) * Eph(k+1,p) ;
    %     T_m_h_r2(k+1,:) = NK * J_vm_h_r2(k+1,:) * Eph(k+1,p) ;
end
...
for p = 1:numel(Pho)
    if(th < Pho(p)/Pmo)
        ...
        Pcov_h_r1_t2(p) =  T2(p).* noise_term_h_r1_t2(p) .* interference_term_h_h_r1_t2(p) .* interference_term_h_m_r1_t2(p);
