function [Pcov_h, Pcov_m] = simulate_downlink_coverage_with_noma_pc(params)

simulation_area =  (params.simulation_area_side(2) - params.simulation_area_side(1))^2;
points = numel(params.NOMA.epsilon);
...
for p = 1:points
    fprintf('\n')
    disp(['NOMA PC ratio: ' , num2str(params.NOMA.epsilon(p))]);
    ...
    for m = 1:params.space_realizations;
        ...
        for t = 1:params.time_slots
            ...
            for user_h = 1:N_users_HTC
                ...
                S_h(user_h) = params.NOMA.epsilon(p) * R_h(server_h,user_h);
                I_h(user_h) = sum(R_h(:,user_h)) -  R_h(server_h,user_h)  + params.NOMA.theta_D * (1 - params.NOMA.epsilon(p)) *  R_h(server_h,user_h);
            end
            SINR_h = S_h ./ (I_h + params.No);   % Noise here is per only one RB !!!!!!
            
            for user_m = 1:N_users_MTC
                server_m = association.MTC_to_BS(user_m);
                S_m(user_m) = (1 - params.NOMA.epsilon(p)) * R_m(server_m,user_m);
                I_m(user_m) = sum(R_m(:,user_m)) - R_m(server_m,user_m) +   params.NOMA.epsilon(p) *  R_m(server_m,user_m);
            end
