function [Pcov_h, Pcov_m] = simulate_uplink_coverage_with_Pho(params)
simulation_area =  (params.simulation_area_side(2) - params.simulation_area_side(1))^2;
points = numel(params.HTC.Pmin);
...
for p = 1:points
fprintf('\n')
disp(['HTC Minimum Power: ' , num2str(params.HTC.Pmin(p))]);
...
for m = 1:params.space_realizations;
    ...
    server_HTC_P = params.HTC.Pmin(p) * exp(params.SEPL.alpha .* distances.HTC_to_Server .^ params.SEPL.beta);
...
%         end
        ...
        for user_m = 1:N_users_MTC
            ...
            Isic(user_m) = H_m(server_m,user_m) * params.HTC.Pmin(p);
        end
