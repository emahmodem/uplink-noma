function [Pcov_h, Pcov_m] = compute_downlink_coverage_with_mtc_density(params)
a = params.SEPL.alpha;
b = params.SEPL.beta;
...
switch (params.aggregation_mode)
    case 'C2A'
        ...
        pa = 1 - po;
        pa = pa * ones(1,numel(params.LA_M));
        ps = pa;
    case 'C2C' 
        k= params.LA_B ./ (params.LA_H + params.rho_m * params.LA_M);
        po = ((3.5 * k) ./ (1 + 3.5 * k)).^ 3.5;
        pa = 1 - po;
        ps = ones(1,numel(params.LA_M));
end
...
for p = 1:numel(params.LA_M)
    Pcov_h(p) =  2 * pi * params.LA_B * integral(@(r)F(r,1,pa(p),params.LA_B,delta_h,NN,a,b,apl_h,1),0,inf);
    Pcov_m(p) =  2 * pi *  ps(p) *  params.LA_B * integral(@(r)F(r,ps(p),pa(p),params.LA_B,delta_m,NN,a,b,apl_m,1),0,inf);
end
